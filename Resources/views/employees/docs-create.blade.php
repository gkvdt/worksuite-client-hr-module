...
<div class="modal-body">
    @if($upload)
        ...
        {!! Form::close() !!}
    @else
        <div class="alert alert-danger">@lang('messages.storageLimitExceed', ['here' => '<a href='.route('client.billing.packages'). '>Here</a>'])</div>
    @endif
</div>
...
<script>
    ...
    function storeDocs(){
        ...
        $('#errorOccasion').html('');
        $('.help-block').remove();
        var url = "{{ route('client.employee-docs.store') }}";
        $.easyAjax({
            type: 'POST',
            url: url,
